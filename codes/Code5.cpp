...
void constructTree(const string& input)
{
    ...
    while (commaPos != string::npos)
    {
        ...
        nodeMap[parent].second.push_back(child);
...
{
    cout << prefix << node;
    if (!nodeMap[node].second.empty())
    {
        cout << " : ";
        for (size_t i = 0; i < nodeMap[node].second.size(); i++)
        {
            cout << nodeMap[node].second[i];
            if (i != nodeMap[node].second.size() - 1)
                cout << ", ";
        }
        cout << endl;
        ...
        for (size_t i = 0; i < nodeMap[node].second.size(); i++)
        {
            printTree(nodeMap[node].second[i], prefix + "    ");
        }
    }
    else
    {
...
bool printPathToTop(const string& node, const string& targetNode, vector<string>& path)
{
    ...
    for (size_t i = 0; i < nodeMap[node].second.size(); i++)
    {
        if (printPathToTop(nodeMap[node].second[i], targetNode, path))
        {
            path.push_back(node);
            return true;
        }
...
{
    for (unordered_map<string, pair<string, vector<string> > >::const_iterator it = nodeMap.begin(); it != nodeMap.end(); ++it)
    {
        if (it->first != node)
        {
            const vector<string>& children = it->second.second;
